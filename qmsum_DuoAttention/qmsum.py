...
def execute_kvpress_request_with_output(row, press) -> tuple[float, float, float, float, str]:
    ...
    context = (
        ...
        "You are an expert at query-based multi-domain meeting summarisation.  "
        "Below is the full transcript of the meeting:\n\n"
        f"{row.context}\n\n"
        f"Query: {row.input}\n\n"
        "Please generate a concise, query-focused summary that directly answers the query.  "
...
def main(input_csv: str, output_csv: str, compression_rate: float):
    # Create DuoAttentionPress with the desired ratio
    press = DuoAttentionPress(head_compression_ratio=compression_rate)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Compression ratio to use for DuoAttentionPress"
    )
