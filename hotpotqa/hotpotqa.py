...
def execute_kvpress_request_with_output(row, press) -> tuple[float, float, float, float, str]:
    ...
    context = (
        ...
        "You are a precise, factual question‑answering assistant capable of multi‑hop reasoning.  "
        "Below is the supporting context and a question based on that context:\n\n"
        f"{row.context}\n\n"
        f"Question: {row.input}\n\n"
        "Using only the information above, supply the correct answer in no more than five words.  "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input-csv", type=str, default="hotpotqa.csv",
